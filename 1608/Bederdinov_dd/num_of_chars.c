...
{
    srand(time(NULL));
    const long long MAX_STRING_SIZE = /*rand() %*/ 1000000000;
    ...
    str_part = MAX_STRING_SIZE / procNum;
    ...
    if (procRank != 0)
    {
        if (procRank == procNum - 1)
            for (i = (procRank * str_part); i < (procRank + 1) * str_part + MAX_STRING_SIZE % procNum; i++)
                char_count++;
        else
            for (i = (procRank * str_part); i < (procRank + 1) * str_part; i++)
                char_count++; 
    }
